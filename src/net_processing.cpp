...
bool TipMayBeStale(const Consensus::Params &consensusParams)
{
    ...
    return g_last_tip_update < GetTime() - (consensusParams.nPowTargetSpacing * 3 * 120) && mapBlocksInFlight.empty(); // 120x faster than bitcoin
}

// Requires cs_main
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    return chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - (consensusParams.nPowTargetSpacing * 20 * 120); // 120x faster than bitcoin
}
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / (chainparams.GetConsensus().nPowTargetSpacing * 120); // 120x faster than bitcoin
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.nPowTargetSpacing * 120); // 120x faster than bitcoin
                nSyncStarted++;
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (nNow > state.nDownloadingSince + (consensusParams.nPowTargetSpacing * 120) * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) { // 120x faster than bitcoin
