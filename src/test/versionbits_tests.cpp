...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    lastBlock = firstChain.Mine(80640, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine 80640-5 more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<80640-4; i++) {
        lastBlock = firstChain.Mine(80640+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime = nStartTime;
    for (int i=80640-4; i<=80640; i++) {
        lastBlock = firstChain.Mine(80640+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(80640*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += 15;
    int blocksToMine = 80640*2; // test blocks for up to 2 time periods
    int nHeight = 80640*3;
    ...
    // next period.
    lastBlock = secondChain.Mine(80640, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(80640*2, nTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine((80640*3)-1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(80640*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
